...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (number === "off") {
    if (bombingFlags[threadID]) {
      ...
      return api.sendMessage("✅ SMS BOMBER 4 Offed ।", threadID);
    } else {
  ...
  if (bombingFlags[threadID]) {
    return api.sendMessage("❗sms bomber 4 already on off korte .sms off", threadID);
  }
 
  api.sendMessage(`✅ SMS BOMBER 4 ON  ${number} ...\nOff korte .sms off`, threadID);
